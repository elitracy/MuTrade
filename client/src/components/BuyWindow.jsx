...
async function updateBuy(sub, coin, quantity) {
  const promise = fetch(
    `https://mutrade.herokuapp.com/api/buy?sub=${sub}&coin=${coin}&quantity=${quantity}`,
    {
      method: "PUT",
    }
  )
  ...
  return (
    <div className="content" class="mb-3">
      ...
      >
        <input
          type="text"
          class="w-1/4 m-2 pl-3 bg-transparent text-white text-2xl text-left outline-none border-b-2 border-white"
          value={buyVal}
          onInput={(e) => {
        ...
        <h1 class="text-center w-1/2 my-auto py-1 h-full pl-4 text-2xl text-gray-300 overflow-scroll">
          ...
          <span class="text-yellow-300">
            $
            {isNaN(parseFloat(buyVal) * 3)
              ? Intl.NumberFormat("en-US").format(0)
              : Intl.NumberFormat("en-US").format(
                  (parseFloat(buyVal) * props.price).toFixed(3)
                )}
          </span>
        </h1>
      </div>
      ...
      >
        <input
          ...
          class="w-full border-2 border-transparent  bg-white py-2 px-3 mt-2 rounded-3xl text-xl text-blue-800 
            transition-colors hover:bg-green-300 cursor-pointer"
          onClick={() => {
            updateBuy(sub, props.coin, buyVal);
            setTimeout((val) => {
